...
const api = axios.create({
  baseURL: 'http://getlinkedbo.cropup.fr/',
});
...
const APIMiddleware = (store) => (next) => (action) => {
  if (action.type === 'FETCH_ARTISTS') {
    ...
    const artistsList = axios.get('http://getlinkedbo.cropup.fr/api/v1/users/');
    const eventsList = axios.get('http://getlinkedbo.cropup.fr/api/v1/events/');
    const stylesList = axios.get('http://getlinkedbo.cropup.fr/api/v1/styles/');
    const categoriesList = axios.get('http://getlinkedbo.cropup.fr/api/v1/categories/');
    ...
    axios.get('http://getlinkedbo.cropup.fr/api/v1/events/')
      .then((response) => {
        store.dispatch({
          type: 'LOAD_EVENTS',
    ...
    axios.post('http://getlinkedbo.cropup.fr/api/v1/events/', {
    ...
    axios.post('http://getlinkedbo.cropup.fr/api/v1/users/', user_type)
      .then(() => {
        window.scroll(0, 0);
    ...
    const state = store.getState();
    axios.put('http://getlinkedbo.cropup.fr/api/v1/users/' + localStorage.getItem('id'), {
    ...
    const state = store.getState();
    const token = null;
    axios.post('http://getlinkedbo.cropup.fr/api/login_check', {
